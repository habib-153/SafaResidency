            ...
            <div className='max-w-screen-3xl'>
                <div className="header-container md:flex items-center justify-between">
                    ...
                    <div className="navigation-buttons flex gap-5 justify-between mt-3">
                        <button className="flex gap-2 group items-center prev " onClick={handlePrev}>
                            <GoHorizontalRule className='group-hover:hidden text-3xl' />
                            <FaArrowLeftLong className="text-3xl hidden group-hover:inline" />
                            Prev
                        </button>
                        <button className="flex group gap-2 next items-center " onClick={handleNext}>
                            Next <FaArrowRightLong className="text-3xl hidden group-hover:inline" />
                ...
                >
                    {categories.map((category, index) => (
                        <SwiperSlide className="swiper-slide flex-col rounded-md cursor-grab active:cursor-grabbing " key={index} tabIndex={0}>
                            <img src={category.image} alt={category.category} className="w-full h-56 object-cover rounded-lg shadow-lg" />

                            <h2 className="text-xl flex gap-3 text-center font-medium my-4 items-center">{category.category}
